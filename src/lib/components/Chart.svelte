<script>
  import { quantize, piecewise, pie, arc } from 'd3';
  
  export let data;
  
  const width = 220 // the outer width of the chart, in pixels
  ...
  const outerRadius = Math.min(width, height) * 0.5 - 80; // the outer radius of the circle, in pixels
  const innerRadius = 110; // the inner radius of the chart, in pixels
  ...
  const strokeColorWOR = 'none'; //stroke color when inner radius is greater than 0
  ...
  const padAngle = stroke === 'none' ? 0 / outerRadius : 0; // angular separation between wedges
  ...
  let colors;
  if (!colors) colors = quantize(piecewise(["#88B04B", "#BB2649", "#0F4C81"]), xVals.length);
